...
public class StopApp implements Runnable {
	
	private volatile Thread blinker;
	private XenVM vm;
	private final String dbCmd = "cd /usr/glassfish3/javadb/bin; ./stopNetworkServer '&'";
	private final String serverCmd = "cd /usr/glassfish3/bin; asadmin stop-domain domain1";
	private final String appCmd = "cd /root/appTest/javapetstore-2.0-ea5; ant -f setup/setup.xml setup; ant run;";
	
	public StopApp(XenVM vm){
		this.vm = vm;
	}
	...
	public void start() {
		System.out.println("StopApp start " + vm.getHostIp());
		blinker = new Thread(this, "StartApp");
		blinker.start();
	}

	public void stop() {
		System.out.println("StopApp stop " + vm.getHostIp());
	...
	public void join() {
		System.out.println("StopApp join " + vm.getHostIp());
		try {
			this.blinker.join();
		} catch (InterruptedException e) {
			e.printStackTrace();
